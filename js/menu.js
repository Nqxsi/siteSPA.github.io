document.addEventListener('DOMContentLoaded', () => {
	...
	const onScrollHeader = () => {
		...
		window.addEventListener('scroll', () => {
			...
			const headerHidden = () =>
				header.classList.contains('headerHidden') // узнаем скрыт header или нет

			if (currentScroll > prevScroll && !headerHidden()) {
				// если прокручиваем страницу вниз и header не скрыт
				header.classList.add('headerHidden') // то скрываем header
			}
			if (currentScroll < prevScroll && headerHidden()) {
				// если прокручиваем страницу вверх и header скрыт
				header.classList.remove('headerHidden') // то отображаем header
			}
